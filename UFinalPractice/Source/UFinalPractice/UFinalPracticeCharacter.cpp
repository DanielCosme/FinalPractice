...
AUFinalPracticeCharacter::AUFinalPracticeCharacter()
{
...
void AUFinalPracticeCharacter::BeginPlay()
{
	// Call the base class  
	Super::BeginPlay();

	auto widget = CreateWidget<UPlayerHpWidget>(UGameplayStatics::GetPlayerController(GetWorld(), 0), wg);
	widget->player = this;
...
void AUFinalPracticeCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (ProjectileClass != NULL)
	{
		...
		if (World != NULL)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<AUFinalPracticeProjectile>(ProjectileClass, SpawnLocation, SpawnRotation);
			}
			else
			{
				...
				World->SpawnActor<AUFinalPracticeProjectile>(ProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
			}
		}
	}
...
void AUFinalPracticeCharacter::MoveRight(float Value)
{
	if (Value != 0.0f)
	{
		// add movement in that direction
...
void AUFinalPracticeCharacter::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void AUFinalPracticeCharacter::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
